...
export const buildPersonSchema = () => ({
  ...
  ...(credentials.certifications.length > 0
    ? {
        hasCredential: credentials.certifications.map(
...
export const buildProfessionalServiceSchema = () => ({
  ...
  alternateName: [
    "Ferdinard Ashonibare Developer",
    "Ashonibare Web Developer",
    "Ferdinard Developer Lagos",
  ],
  url: SITE_URL,
  description:
    "Full-stack web developer in Lagos, Nigeria building fast, secure web platforms for government, hospitality, education, and real estate.",
  image: `${SITE_URL}${siteConfig.schemaImage || siteConfig.defaultImage}`,
  email: siteConfig.email,
  telephone: siteConfig.phone,
  address: {
  ...
  hasOfferCatalog: {
    ...
    itemListElement: [
      {
        "@type": "Offer",
        itemOffered: { "@type": "Service", name: "Websites & Brand Sites" },
      },
      {
        "@type": "Offer",
        itemOffered: { "@type": "Service", name: "Web Applications" },
      },
      {
        "@type": "Offer",
        itemOffered: { "@type": "Service", name: "APIs & Backends" },
      },
      {
        "@type": "Offer",
        itemOffered: { "@type": "Service", name: "Security & Performance" },
      },
    ],
  },
  priceRange: "$$",
  ...
  if (isArticle) {
    itemListElement = [
      { "@type": "ListItem", position: 1, name: "Home", item: SITE_URL },
      {
        ...
        position: 2,
        name: "Insights",
        item: toAbsoluteUrl("/insights"),
      },
      {
        "@type": "ListItem",
        position: 3,
        name: route.label,
        item: toAbsoluteUrl(route.path),
      },
    ];
  } else if (isCaseStudy) {
    itemListElement = [
...
export const buildProjectCaseStudySchema = (project) => ({
  ...
  image: project.image
    ...
    : `${SITE_URL}${siteConfig.defaultImage}`,
  author: { "@id": PERSON_ID },
  publisher: { "@id": ORGANIZATION_ID },
